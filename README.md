...
2. Open the .zip archive, copy `winmm.dll` into the game directory. You can navigate to the game's directory by right-clicking the game in Steam, hovering "Manage", and selecting "Browse local files". If you are upgrading from an older version of Lovely, delete the old `version.dll` from the game directory.
